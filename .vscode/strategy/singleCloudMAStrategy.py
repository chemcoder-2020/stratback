...
class SingleCloudMAStrategy(Strategy):
    ...
    def next(self):
        if self.eod:
            ...
            if crossover(self.longs, 0.5):
                limit = (
                    (1 - self.limit_pct) * self.close
                    if self.limit_pct is not None
                    else None
                )
                # stop = (1-self.stop_pct) * limit if self.stop_pct is not None else None
                # tp = (1 + self.pl_pct_tp) * limit if self.pl_pct_tp is not None else None
                ...
                self.position.close()
                self._signals[self.data.index[-1]] = -2
            elif self.pl_pct_tp is not None:
