...
def compute_stats(
    ...
    s.loc["Max. Drawdown [%]"] = np.abs(max_dd * 100)
    s.loc["Avg. Drawdown [%]"] = np.abs(-dd_peaks.mean() * 100)
    ...
    s.loc["Avg. Win [%]"] = returns[returns.gt(0)].mean() * 100
    s.loc["Avg. Loss [%]"] = np.abs(returns[returns.lt(0)].mean() * 100)
    s.loc["Avg. Trade WL Ratio"] = s.loc["Avg. Win [%]"] / s.loc["Avg. Loss [%]"]
    s.loc["Max. Trade Duration"] = _round_timedelta(durations.max())
    s.loc["Avg. Trade Duration"] = _round_timedelta(durations.mean())
    s.loc["Profit Factor"] = returns[returns > 0].sum() / (
        abs(returns[returns < 0].sum()) or np.nan
    )  # noqa: E501
    s.loc["Ehlers Criterion"] = (2 * win_rate - 1) * s.loc["Profit Factor"]
    s.loc["Ehlers Criterion 2"] = (2 * win_rate - 1) * s.loc["Equity Final [$]"]
    s.loc["Ehlers Criterion 3"] = (2 * win_rate - 1) * s.loc["Equity Final [$]"] * s.loc["Profit Factor"]
    s.loc["Ehlers Criterion 4"] = (2 * win_rate - 1) * s.loc["Return (Ann.) [%]"] * s.loc["Profit Factor"]
    s.loc["Ehlers Criterion 5"] = (np.int64(np.sign(2 * win_rate - 1)) & np.int64(np.sign(2*s.loc["Avg. Trade WL Ratio"]-2))) * (2 * win_rate - 1) * (2*s.loc["Avg. Trade WL Ratio"]-2)
    s.loc["Ehlers Criterion 6"] = (np.int64(np.sign(2 * win_rate - 1)) & np.int64(np.sign(2*s.loc["Profit Factor"]-2))) * (2 * win_rate - 1) * (2*s.loc["Profit Factor"]-2)
    s.loc["Expectancy [%]"] = returns.mean() * 100
    s.loc["SQN"] = np.sqrt(n_trades) * pl.mean() / (pl.std() or np.nan)
    s.loc["SQN_exp"] = (
        (1 - np.exp(-n_trades / 100)) * pl.mean() / (pl.std() or np.nan)
    )  # reduce the effect of growing number of trades on SQN
