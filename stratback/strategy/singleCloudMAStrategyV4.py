...
from stratback.utils.TALibrary import (
    vwap, price_position_by_pivots
)
import datetime


class SingleCloudMAStrategyV4(Strategy):
    n1 = 11
    n2 = 23
    size = 1
    target_pct = 0.7
    price_position_threshold = 7
    long_only = True
    short_only = False
    ...
    def ma_single_cloud_signal(
        ...
        price_position = price_position_by_pivots(data, secondary_tf=self.secondary_tf)
        ...
        # ma3 = pt.sma(price, ma_length1)
        ma_bandwidth = ma1 - ma2
        good_price_long = price_position.le(self.price_position_threshold)
        good_price_short = price_position.gt(self.price_position_threshold)
        ...
        rsi_slope = pt.linreg(rsi, 5, slope=True)

        # price_rank = anchored_price.groupby(anchored_price.index.to_period("D")).quantile(self.price_quantile)
        ...
        longCondition = (
            ...
            & good_price_long & rsi_slope.gt(0)
        ) | newday_trendup_continuation

        shortCondition = (
            (trend_dn & ~trend_dn.shift().fillna(False))
            & good_price_short & rsi_slope.lt(0)
        ) | newday_trenddn_continuation
