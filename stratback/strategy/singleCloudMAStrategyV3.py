...
class SingleCloudMAStrategyV3(Strategy):
    n1 = 11
    n2 = 23
    rsi_reg = 5
    size = 1
    ...
    def ma_single_cloud_signal(
        ...
        rsi_slope = pt.linreg(rsi, self.rsi_reg, slope=True)
        ...
        longCondition = (
            (trend_up & ~trend_up.shift().fillna(False)) & rsi_slope.gt(0)
        ) | newday_trendup_continuation

        shortCondition = (
            (trend_dn & ~trend_dn.shift().fillna(False)) & rsi_slope.lt(0)
        ) | newday_trenddn_continuation
